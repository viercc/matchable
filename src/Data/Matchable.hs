...
--
-- == Deriving instance using Generics
-- 
--
...
deriving via
  (Generically1 (Const k))
  instance
    (Eq k) => Matchable (Const k)

deriving via
  (Generically1 (Product f g))
  instance
    (Matchable f, Matchable g) => Matchable (Product f g)

deriving via
  (Generically1 (Sum f g))
  instance
    (Matchable f, Matchable g) => Matchable (Sum f g)

deriving via
  (Generically1 (Compose f g))
  instance
    (Matchable f, Matchable g) => Matchable (Compose f g)

instance Matchable Proxy where
  zipMatchWith _ _ _ = Just Proxy

deriving via
  (Generically1 (Tagged t))
  instance
    Matchable (Tagged t)
...
{-# INLINEABLE genericZipMatchWith #-}

instance Matchable V1 where
  {-# INLINEABLE zipMatchWith #-}
...
-- | Until /version 1.2.1/, @Matchable@ and @Matchable'@ was two separate classes with
--   almost equal functionality and instances.
--
--   It is no longer the case, but the name @Matchable'@ is exported to keep
